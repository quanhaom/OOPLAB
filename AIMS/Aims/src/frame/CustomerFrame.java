...
public class CustomerFrame extends BaseFrame {
    ...
    private List<DVD> selected = new ArrayList<>();
    private DVD dvd;
    ...
    private void addDVDToCart() {
    	if(selected.isEmpty()) {
	        ...
	        if (selectedRow != -1) {
	            String dvdId = (String) tableModel.getValueAt(selectedRow, 0);
		   		addDVDToCart(dvd.getDVDById(dvdId));
    		...
    		addDVDToCart(selected);
    		selected.clear();
    	}
    }
    
    private void addDVDToCart(DVD adddvd) {
        for (DVD dvd : cart.keySet()) {
            if (dvd.getId().equals(adddvd.getId())) {
    ...
    private void addDVDToCart(List<DVD> adddvds) {
    	for (DVD a: adddvds) {
            for (DVD dvd : cart.keySet()) {
                if (dvd.getId().equals(a.getId())) {
                    cart.put(dvd, cart.get(dvd) + 1);
                    JOptionPane.showMessageDialog(this, dvd.getTitle() + " quantity increased to " + cart.get(dvd));
